...
Logger.prototype = {
  ...
  callStart: function (details) {
    ...
    var callLocation = stringUtil.extractFile(details.callStack && details.callStack[0]);
    if (this._detailed) {
    ...
    var rightText = callLocation;
    this._print(depth, SYMBOLS.CALL_ENTER, methodName, rightText, true, null);
  },
  ...
  callEnd: function (details) {
    ...
    var execLocation = stringUtil.extractFile(details.execStack && details.execStack[0]);
    if (hasError) {
    ...
    var time = stringUtil.formatRuntime(details.runTime);
    var rightText = execLocation ? (execLocation + ' - ' + time) : time;
    this._print(depth, symbol, message, rightText, true, hasError ? 'yellow' : null);
  }
};
